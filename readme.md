...
* There is no parameter interface in these builds, but the text frontend still understands most of the inline tilde commands (~r rate, ~f pitch, ~g gain, ~u unvoiced gain and ~e excitation including whisper all work), which is how the wrapper and the NVDA addon apply settings to them. The ~v headsize and ~h inflection commands are ignored by the v2 dlls (verified empirically: any value produces byte identical audio), so the NVDA addon hides those two sliders while a v2 language is active. Arabic's dll is a stub that only ever emits silence, so it isn't offered.
